...
# Pre-flight count so the operator sees what will (or did) happen.
COUNT=$(find "$DATA_DIR" -type f -name '*.json' -mtime "+${RETENTION_DAYS}" 2>/dev/null | wc -l | tr -d ' ')
...
if [ "$DRY_RUN" = "1" ]; then
  ...
  find "$DATA_DIR" -type f -name '*.json' -mtime "+${RETENTION_DAYS}" -print
  exit 0
fi
...
echo "pruned $COUNT consent record(s) older than ${RETENTION_DAYS} days from $DATA_DIR"
